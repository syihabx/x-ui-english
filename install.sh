...
download_xui(){
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://syihabul.my.id/x-ui-linux-amd64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://syihabul.my.id/x-ui-linux-amd64.tar.gz"
